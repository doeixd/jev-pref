...
export async function doctor(argv, { cwd = ".", out = console } = {}) {
  ...
  if (config) {
    ...
    if (verbose) {
      ...
      const { sources, ignored, fencedFile, configPath, localConfigPath, localConfigFound } = await resolveConfig({
        rootDir: cwd,
        flags: configFlag !== undefined ? { config: configFlag } : {},
      });
      ...
      push("prefs count", true, `${config.prefs.length} [${config.prefs.map((p) => `${typeof p.name === "string" && p.name.length > 0 ? `${p.name} (${p.id})` : p.id}:${p.type === "choice" ? "choice" : p.gate ? "gate" : "adv"}${p.scope === "change" ? "@change" : ""}`).join(", ")}]`);
